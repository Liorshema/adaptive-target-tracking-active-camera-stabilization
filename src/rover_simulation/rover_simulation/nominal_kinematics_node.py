...
from rover_dynamics.robot_config import load_rover_parameters
...
class NominalKinematicsNode(Node):
    ...
    def __init__(self):
        ...
        config_path = (
            Path(get_package_share_directory('my_robot_pkg'))
            / 'config'
            / 'robot'
            / 'rover.yaml'
        )

        params = load_rover_parameters(config_path)

        # --------------------------------------------------
        # Stage 2 kinematic model
        # --------------------------------------------------

        self.model = RoverKinematicModel(
